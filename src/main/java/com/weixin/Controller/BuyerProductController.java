...
import com.weixin.Utils.ResultVOUtil;
...
public class BuyerProductController {
    ...
    public ResultVO list(){
        ...
        List<ProductCategory> productCategoryList=categoryService.findByCategoryTayeIn(categoryTypeList);

        //3.数据拼装
        List<ProductVO> productVOList=new ArrayList<>();
        for(ProductCategory productCategory:productCategoryList){
            ProductVO productVO=new ProductVO();
            productVO.setCategorytype(productCategory.getCategoryTaye());
            productVO.setCategoryname(productCategory.getCategoryName());


            List<ProductInfoVO> productInfoVOList=new ArrayList<>();
            for(ProductInfo productInfo:productInfoList){
                if (productInfo.getCategoryType().equals(productCategory.getCategoryTaye())){
